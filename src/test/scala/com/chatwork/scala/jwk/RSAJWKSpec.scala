...
class RSAJWKSpec extends FreeSpec with RSAJWKJsonImplicits {
  ...
  "RSAJWK" - {
    "should be able to full construct & serialization" in {
      ...
      val x5t = Base64String("abc", true)
      val x5t256 = Base64String("abc256", true)
      val x5c = List(Base64String("def"))

      val factory = KeyFactory.getInstance("RSA")
      val privateKey = for {
        _n <- Base64String(n, true).decodeToBigInt.map(_.bigInteger)
        _d <- Base64String(d, true).decodeToBigInt.map(_.bigInteger)
      } yield {
